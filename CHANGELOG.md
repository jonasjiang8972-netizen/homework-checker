...
#### 🔧 用户认证修复
- [ ] 创建 SessionProvider 客户端组件
- [ ] 修复 Google OAuth 登录全流程
- [ ] 登录态 UI（用户信息/退出）

#### 🔑 API Key 加密管理
- [ ] AES-256-GCM 加密工具（`lib/encryption.ts`）
- [ ] `user_settings` 表：加密 Key + 偏好
- [ ] API Key CRUD 接口（加密存储/脱敏查询/删除）
- [ ] correct/plans/quiz 路由接入用户 Key

#### 👤 用户中心
- [ ] `/settings` 页面：Key 管理 + 偏好设置 + 统计
- [ ] 数据隔离：登录用户只看自己数据
