...
- `handleSubmit`：压缩→FormData→fetch `/api/correct`，区分 `data.error` 与 `data.result`。
...
响应:   200 { result } | 503/400/413/502 { error }
```
...
6. **结构化提示词**：约束输出为 ①错误之处 ②正确解答 ③错因分析 ④涉及知识点；全对则回"全部正确"。
7. **错误分类**：catch 中识别 timeout 关键字 → "批改超时，请重试"；其余 → "批改服务暂时不可用"。
8. `max_tokens` 1500 适配结构化长输出。
