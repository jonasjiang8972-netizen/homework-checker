...
3. **图存在性**：`formData.get('image')` 为空 → 400。
4. **大小校验**：>10MB → 413。
5. **MIME 兜底** `getMimeType()`：`image.type` 为空时按扩展名推 png/jpeg，解决部分机型不传 type。
6. **视觉预检**：先用轻量模型 Qwen3-VL-8B 判断图片是否可批改，15s 超时
7. **视觉批改**：通过后调用用户选择的视觉模型（默认 Qwen3-VL-32B），120s 超时
8. **OCR 回退**：视觉失败后走 Tesseract.js OCR → 文本模型 Qwen3-32B，60s 超时
9. **结构化提示词**：约束输出为 JSON，含 `guidance`（引导提示）字段
10. **错误分类**：catch 中识别 timeout 关键字 → "批改超时，请重试"；其余 → "批改服务暂时不可用"。
11. **重试机制**：在批改超时或网络中断时自动重试 1 次。
12. `max_tokens` 2000 适配结构化长输出（含引导+完整解答）。
